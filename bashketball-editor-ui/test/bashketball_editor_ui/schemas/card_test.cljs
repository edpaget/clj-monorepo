(ns bashketball-editor-ui.schemas.card-test
  (:require
   [bashketball-editor-ui.schemas.card :as card]
   [cljs.test :as t :include-macros true]
   [malli.core :as m]))

(t/deftest player-card-valid-test
  (t/testing "valid player card passes validation"
    (let [valid-card {:card-type :card-type/PLAYER_CARD
                      :name "Star Player"
                      :sht 5
                      :pss 4
                      :def 3
...
(t/deftest player-card-requires-name-test
  (t/testing "player card requires name field"
    (let [invalid-card {:card-type :card-type/PLAYER_CARD
                        :sht 5
                        :pss 4
                        :def 3
                        :speed 6
...
(t/deftest play-card-valid-test
  (t/testing "valid play card passes validation"
    (let [valid-card {:card-type :card-type/PLAY_CARD
                      :name "Quick Pass"
                      :fate 2
...
(t/deftest game-card-multi-dispatch-test
  (t/testing "GameCard dispatches to correct schema"
    (let [player {:card-type :card-type/PLAYER_CARD
                  :name "Player"
...
(t/deftest valid-fn-test
  (t/testing "valid? helper function works"
    (let [valid-card {:card-type :card-type/COACHING_CARD
                      :name "Timeout"
                      :fate 1
...
(t/deftest explain-fn-test
  (t/testing "explain returns nil for valid card"
    (let [valid-card {:card-type :card-type/TEAM_ASSET_CARD
                      :name "Home Court"
                      :fate 0
