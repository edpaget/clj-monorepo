...
(gql/defresolver :Query :cards
  ...
  [:=> [:cat :any [:map {:optional true}
                   ...
                   [:card-type {:optional true} schemas/CardType]
                   [:offset {:optional true} :int]
                   [:limit {:optional true} :int]] :any]
   schemas/CardsResponse]
  [ctx {:keys [set-slug card-type offset limit]} _value]
  (let [card-type-kw (when card-type
                       (keyword "card-type" (name card-type)))
        opts         (cond-> {}
                       set-slug (assoc-in [:where :set-slug] set-slug)
    ...
    {:data      paginated
     :page-info {:total    total
                 :offset   offset
                 :limit    limit
                 :has-more (< (+ offset (count paginated)) total)}}))
...
(gql/defresolver :Mutation :updateCard
  ...
  Looks up the card by slug and set-slug, then applies the update fields.
