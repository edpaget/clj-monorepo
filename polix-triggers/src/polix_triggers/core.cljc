(ns polix-triggers.core
  ...
  polix-triggers connects events to effects via conditions evaluated by polix.
  The library provides mechanics for trigger registration, event processing,
  timing, and stacking—consumers define their own event types, entities, and
  domain logic.
  ...
  - **Condition**: A polix policy expression for filtering events
  ...
  (:require [polix-triggers.condition :as condition]
            [polix-triggers.processing :as processing]
...
(defn register-trigger
  ...
  - `:doc/self`, `:doc/owner`, `:doc/source` - trigger bindings
  - `:doc/event-type` - the event's type
  - Other event fields without prefix
  ...
  (let [compiled-fn (when-let [cond (:condition trigger-def)]
                      (condition/compile-condition cond))]
    (registry/register-trigger
     registry
     (cond-> trigger-def
       compiled-fn (assoc :condition-fn compiled-fn))
     source-id
     owner
     self)))
