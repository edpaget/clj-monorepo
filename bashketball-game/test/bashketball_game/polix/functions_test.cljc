...
(defn- test-opts
  "Creates opts map with empty registry for function tests."
  []
...
(deftest step-cost-no-zoc-test
  (let [game (-> (fixtures/base-game-state)
        ...
        cost (f game {} (test-opts) fixtures/home-player-1 [2 4])]
...
(deftest step-cost-with-zoc-test
  (let [game (-> (fixtures/base-game-state)
        ...
        f    (functions/get-fn :bashketball-fn/step-cost)
        cost (f game {} (test-opts) fixtures/home-player-2 [2 4])]
...
(deftest zoc-defender-ids-test
  (let [game      (-> (fixtures/base-game-state)
        ...
        defenders (f game {} (test-opts) fixtures/home-player-2 [2 4])]
