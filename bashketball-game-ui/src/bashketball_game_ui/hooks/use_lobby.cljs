  ...
  (let [[create-fn ^js result] (useMutation
    ...
    [(fn [deck-id]
     ...
     {:loading (.-loading result)
      :error   (.-error result)
      :data    (some->> ^js result .-data .-createGame
...
(defn use-join-game
  ...
  []
  (let [[join-fn ^js result] (useMutation
    ...
    [(fn [{:keys [game-id deck-id]}]
     ...
     {:loading (.-loading result)
      :error   (.-error result)
      :data    (some->> ^js result .-data .-joinGame
                        (decoder/decode game-schema/GameSummary))}]))
...
(defn use-leave-game
  ...
  []
  (let [[leave-fn ^js result] (useMutation
...
(defn use-forfeit-game
  ...
  takes a game-id string."
  []
  (let [[forfeit-fn ^js result] (useMutation
    ...
    [(fn [game-id]
     ...
     {:loading (.-loading result)
      :error   (.-error result)
      :data    (some->> ^js result .-data .-forfeitGame
